...
function copyPassword() {
    const display = getPasswordDisplay();
    const text = manualPasswordCheckbox?.checked
...
function switchTab(tab) {
    ...
    noteContent.classList.toggle('hidden', tab !== 'note');
    ...
    if (secondContainer) {
        secondContainer.classList.toggle('hidden', tab === 'note');
    }
...
async function saveEmployeePassword() {
    ...
    const password = manualPasswordCheckbox?.checked
        ? (manualPasswordField.value.trim())
        : (passwordDisplayEmployee ? passwordDisplayEmployee.textContent : '');

    if (!password) { showMessage('Сначала создайте пароль', 'error'); return; }
...
async function saveServicePassword() {
    ...
    const password = passwordDisplayService ? passwordDisplayService.textContent : '';
    if (!password) { showMessage('Сначала создайте пароль', 'error'); return; }
...
function resetPasswordDisplay() {
    if (passwordDisplayEmployee) { passwordDisplayEmployee.textContent = ''; passwordDisplayEmployee.classList.add('hidden'); }
    if (passwordDisplayService)  { passwordDisplayService.textContent  = ''; passwordDisplayService.classList.add('hidden'); }
    if (manualPasswordCheckbox) manualPasswordCheckbox.checked = false;
...
/* =============== Manual password toggle (employee only) =============== */
...
manualPasswordCheckbox?.addEventListener('change', toggleManualPassword);
